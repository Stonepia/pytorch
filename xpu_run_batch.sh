...
WORKSPACE=`pwd`
LOG_DIR=$WORKSPACE/inductor_log/${SUITE}/${MODEL_ONLY}/${DT}
mkdir -p ${LOG_DIR}
LOG_NAME=inductor_${SUITE}_${MODEL_ONLY}_${DT}_${MODE}_${DEVICE}_${SCENARIO}
...
export TORCH_COMPILE_DEBUG_DIR=${LOG_DIR}/torch_compile_debug
mkdir -p ${TORCH_COMPILE_DEBUG_DIR}
export TRITON_DEBUG_DIR=${LOG_DIR}/triton_debug
mkdir -p ${TRITON_DEBUG_DIR}
...
ulimit -n 1048576
if [[ $DT == "amp_bf16" ]]; then
    ZE_AFFINITY_MASK=${CARD} python benchmarks/dynamo/${SUITE}.py --${SCENARIO} --amp -d${DEVICE} -n10 --no-skip --dashboard ${Mode_extra} ${Shape_extra} ${partition_flags} ${Model_only_extra} --backend=inductor --export-profiler-trace --timeout=4800 --output=${LOG_DIR}/${LOG_NAME}.csv 2>&1 | tee ${LOG_DIR}/${LOG_NAME}.log
